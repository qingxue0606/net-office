        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                if (e.Row.RowState == DataControlRowState.Normal || e.Row.RowState == DataControlRowState.Alternate)
                {
                    ...
                    ((LinkButton)e.Row.Cells[1].Controls[0]).Attributes.Add("onclick", "showSealImg(this," + e.Row.Cells[0].Text + ")");
                    ((LinkButton)e.Row.Cells[1].Controls[0]).Attributes.Add("href", "#");


                    //byte[] dt = sealMg.GetQueryCollection(" order by ID desc")[e.Row.RowIndex].SealImage;
                    //string s = Convert.ToBase64String(dt);
                    //((LinkButton)e.Row.Cells[1].Controls[0]).Attributes.Add("value", "data:" + sealMg.GetQueryCollection(" order by ID desc")[0].SealImageType + ";base64," + s);
